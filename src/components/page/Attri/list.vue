  ...
  <div class="list">
    <div class="crumbs">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item><i class="el-icon-lx-home"></i> 属性管理</el-breadcrumb-item>
        <el-breadcrumb-item> {{$route.meta.title}}</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="container">
      <div class="handle-box">
        <el-button type="primary" icon="el-icon-plus" @click="addCounty(0,0)">添加属性</el-button>
      </div>
      <el-table
      :data="tableData"
  ...
  export default {
    ...
    methods: {
      async init() {
        ...
        let rest = await this.$axios.AttriDetails(param);
      ...
      changeOrder() {
        this.$refs.form.validate(async (valid) => {
          if (valid) {
            ...
            if (this.form.id > 0) {
              let rest = await this.$axios.editAttriDetails(param);
              let {code, result, msg} = rest;
              if (code == 200) {
              ...
              param.id = '';
              let rest = await this.$axios.addAttriDetails(param);
              let {code, result, msg} = rest;
              if (code == 200) {
                this.$message.success('添加成功');
                this.showSure = false;
      ...
      delItem(id) {
        this.$confirm('此操作将永久删除, 是否继续?', '提示', {
          ...
          let rest = await this.$axios.deleteAttriDetails(param);
          let {code, result, msg} = rest;
          if (code == 200) {
